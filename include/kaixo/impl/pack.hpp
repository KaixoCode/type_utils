...
namespace kaixo {
    ...
    namespace pack {
        ...
        namespace detail {
            ...
            template<template<class...> class Ty, class A, class B, structured_binding C>
            ...
            template<template<class...> class Ty, structured_binding B>
            ...
            template<template<auto, auto, class...> class Ty, auto A, auto B, structured_binding C>
            ...
            template<template<auto, class, class...> class Ty, auto A, class B, structured_binding C>
        ...
        namespace detail {
            ...
            struct first_indices_impl {
                constexpr static std::array<std::size_t, count_unique_v<Args...>> value = []{
                    std::array<std::size_t, count_unique_v<Args...>> _result{};
                    std::size_t _index = 0, _match = 0;
                    (((index_v<Args, Args...> == _index) ? _result[_match++] = _index++ : ++_index), ...);
                    return _result;
                }();
            };
        }
        ...
        template<class ...Tys>
        struct first_indices : detail::apply_pack_d<detail::first_indices_impl, Tys...> {};

        template<>
        struct first_indices<> {
            template<class ...Tys>
            using type = first_indices<Tys...>;
            constexpr static type_filter<type> value{};
        };
         ...
         */
        template<class ...Tys>
        constexpr auto first_indices_v = first_indices<Tys...>::value;
        ...
        namespace detail {
            ...
            struct unique_impl_base<std::index_sequence<Is...>, Tys...> {
                using type = info<element_t<first_indices_v<Tys...>[Is], Tys...>...>;
            };
    ...
    consteval std::size_t count_filter_impl(info<Args...>) {
        return ((static_cast<std::size_t>(matches_filter<Filter, 
                typename Args::template element<1>::type, // Type
                Args::template element<0>::value>         // Index
            )) + ... + 0);
    }
    ...
    struct count_filter {
        ...
        struct helper<std::index_sequence<Is...>> {
            consteval static std::size_t value() {
                return ((static_cast<std::size_t>(matches_filter<Filter, Args, Is>)) + ... + 0);
            }
        };
